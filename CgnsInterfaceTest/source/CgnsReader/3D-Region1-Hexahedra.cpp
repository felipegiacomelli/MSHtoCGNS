...
struct Region1_Hexahedra_3D_Cgns {
	Region1_Hexahedra_3D_Cgns() {
		CgnsReader3D cgnsReader3D(std::string(TEST_INPUT_DIRECTORY) + "CgnsInterface/3D-Region1-Hexahedra/27v_8e.cgns");
...
TestCase(Geometry) {
	...
	checkEqual(region.elementEnd, 8);
}
...
TestCase(South) {
	...
	checkEqual(boundary.facetEnd, 20);
...
TestCase(North) {
	...
	checkEqual(boundary.facetBegin, 20);
	checkEqual(boundary.facetEnd, 24);
